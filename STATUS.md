...
**Обновлено:** 2026-07-28 (сессия: tooltip fix, version bump, AddTransactionDialog refactor)  
...
## Что сделано в последней сессии (Kimi)

| # | Файл | Изменение |
|---|------|-----------|
| 1 | `wesi_tooltip.dart` | Исправлено "застревание" подсказок — отмена delayed future при `onExit` |
| 2 | `settings_screen.dart` | Версия обновлена с `v0.1.1 α` → `v0.1.2 α` |
| 3 | `add_transaction_dialog.dart` (новый) | `AddTransactionDialog` вынесен из `treasury_screen.dart` в публичный файл |
| 4 | `treasury_screen.dart` | Использует публичный `AddTransactionDialog` вместо private класса |
...
| 8 | Home **Продажа** → сразу экран/диалог продажи | 🔄 | `AddTransactionDialog` вынесен в публичный файл. Нужно обновить `home_screen.dart` чтобы открывал диалог напрямую. |
