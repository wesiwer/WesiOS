...
**Обновлено:** 2026-07-28 (сессия 2, часть 4: мульти-движковый прогноз — Wesi Horizon / Prophet / SARIMAX)  
**Репо:** https://github.com/wesiwer/WesiOS · **ветка:** claude/document-review-wp4jch · **UI:** v0.4.0 α · **pubspec:** 0.4.0+6  
...
### Bundling Python — где именно провели черту, и почему

Существующий Windows-инсталлятор весит **~15 МБ**. Замерил реальный вес
каждого варианта локально (pip install на этой машине) перед тем, как
решать за пользователя:

| Стек | Вес | Риск сборки |
|---|---|---|
| numpy + pandas + statsmodels (SARIMAX) | **~350 МБ** | Низкий — чистые wheels, без компиляции, ставится за секунды даже на Windows |
| + prophet + cmdstanpy (Prophet) | **ещё +~1.2 ГБ** (итог ~1.7 ГБ) | Высокий — cmdstanpy собирает CmdStan из исходников (C++ toolchain), **4+ минуты** на этой машине (Linux); на Windows CI может быть дольше/капризнее (MinGW/MSVC toolchain на раннере не гарантирован) |

Решение (без дополнительного подтверждения — это тюнинг уже одобренного
пользователем выбора «подключить Prophet/SARIMAX», не новая архитектура):
**в `build.yml` бандлится только SARIMAX-стек** (embeddable Python 3.11 +
pip + numpy/pandas/statsmodels, шаг «Bundle Python runtime for the SARIMAX
forecast engine»). `forecast_prophet.py` в бандл тоже кладётся (несколько
КБ, не жалко), но prophet/cmdstanpy **не устанавливаются** — если запустить
без них, скрипт падает на `import prophet` (ImportError в stderr, пустой
stdout), `ExternalForecastBridge` уже трактует это как «недоступен» (null),
UI показывает тумблер Prophet как недоступный с объясняющим тултипом —
честно, не падает.

**Prophet по умолчанию не бандлится — это открытое решение, ждёт
пользователя** (цифры см. таблицу выше): либо смириться с ~1.7 ГБ
инсталлятором по умолчанию, либо сделать Prophet опциональной догрузкой
(кнопка в Settings «Включить Prophet» → скачивает рантайм отдельно), либо
не бандлить вовсе (работает только если у пользователя технически стоит
свой Python с prophet — тот же dev-fallback путь, что используется сейчас
при локальной разработке). См. финальный ответ пользователю.

### Область действия — Windows-only

Prophet/SARIMAX/Combined **работают только в Windows-сборке**. На Android
`ExternalForecastBridge._resolvePython()` не найдёт ни бандл (его там нет и
не будет — subprocess-Python на Android не то же самое, что на десктопе),
ни системный `python3` — тумблеры честно покажут «недоступен», доступен
только Wesi Horizon. Это не баг, а следствие того, что бандлинг Python
подпроцессом — desktop-специфичный паттерн; для Android потребовался бы
принципиально другой подход (например, Chaquopy или облачный API) — новое
архитектурное решение, не часть этого захода.
