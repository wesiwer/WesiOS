...
function access(e, ctx) {
  let permissions = {};
  if (!ctx.isOwner) {
    const employee = dataAccess.first(e.app, "wesios_records", "owner={:owner} && coll='employees' && rid={:rid} && deleted=false", {owner: ctx.ownerId, rid: ctx.employeeId});
...
module.exports = {
  ...
  execute: function(e, ctx, name, args) {
    ...
    if(name.startsWith("crm_client_")){
      ...
      if(name==="crm_client_archive"){const saved=syncWriter.write(e,ctx,{coll:"crm_clients",rid:id,before:before,next:before,deleted:true});if(!saved.applied)return{ok:false,code:"WRITE_CONFLICT",message:"CRM-клиент уже изменился, повторите действие"};return{ok:true,result:{client:{id,archived:true}}};}
      ...
      next.updatedAt=now;const saved=syncWriter.write(e,ctx,{coll:"crm_clients",rid:id,before:before,next:next});if(!saved.applied)return{ok:false,code:"WRITE_CONFLICT",message:"CRM-клиент изменился параллельно, повторите действие"};const out=saved.payload;return{ok:true,result:{client:{id,name:String(out.name||""),status:String(out.status||"lead")}}};
    }
