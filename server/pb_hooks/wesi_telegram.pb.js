const tg = require(`${__hooks}/wesi_telegram_lib.js`);
const store = require(`${__hooks}/wesi_telegram_store.js`);
const financeTools = require(`${__hooks}/wesi_ai_finance_tools.js`);
const horizonTools = require(`${__hooks}/wesi_ai_horizon_tools.js`);
const taskTools = require(`${__hooks}/wesi_ai_task_tools.js`);

const RATE_LIMIT = 24;
const RATE_WINDOW_MS = 60 * 1000;
const ALERT_CRON_ID = "wesios_telegram_alerts_v1";
...
function visibleSelection(app, identity, requested) {
  ...
  if (wanted && selected.id !== wanted) {
    return {ok: false, code: "FORBIDDEN", message: "Нет доступа к этой организации", organizations: selected.organizations};
  }
  if (!selected.id) return {ok: false, code: "FORBIDDEN", message: "Нет доступной организации", organizations: selected.organizations};
...
function telegramApi(cfg, method, payload) {
  ...
  if (!response || response.statusCode < 200 || response.statusCode >= 300 || data.ok !== true) {
    return {ok: false, code: "TELEGRAM_BAD_RESPONSE", description: String(data.description || "")};
  }
...
function deepLinkUrl(cfg, target, organizationId) {
  const t = String(target || "home").replace(/[^a-z0-9_-]/gi, "").slice(0, 24) || "home";
  const org = encodeURIComponent(String(organizationId || ""));
  return `${cfg.publicBaseUrl}/api/wesi/telegram/open?target=${encodeURIComponent(t)}&org=${org}`;
}
...
function orgKeyboard(organizations, activeId) {
  ...
  for (const org of source) {
    const label = (String(org.id || "") === String(activeId || "") ? "✓ " : "") + String(org.name || org.id || "org");
    buttons.push([{text: label.slice(0, 52), callback_data: tg.callback("org", String(org.id || ""))}]);
  }
  return buttons;
}
...
function safeToolCall(fn) {
  try { return fn(); }
  catch (_) { return {ok: false, code: "DATA_UNAVAILABLE", message: "Данные WesiOS временно недоступны"}; }
}

function financeBalance(e, ctx, organizationId) {
  return safeToolCall(() => financeTools.execute(e, ctx, "finance_balance", {}, organizationId));
}

function horizonSnapshot(e, ctx, organizationId) {
  return safeToolCall(() => horizonTools.execute(e, ctx, "horizon_snapshot", {}, organizationId));
}

function dueTasks(e, ctx, organizationId, mode, limit) {
  return safeToolCall(() => taskTools.execute(e, ctx, "tasks_list", {
    dueMode: mode,
...
function orgName(app, identity, organizationId) {
  ...
  const hit = organizations.find((item) => String(item.id || "") === String(organizationId || ""));
...
function renderTaskList(title, result, organization, maxItems) {
  if (!result || result.ok !== true) {
    const message = result && result.code === "FORBIDDEN" ? "Нет доступа к задачам этой организации." : "Задачи сейчас недоступны.";
    return `<b>${tg.escapeHtml(title)}</b>\n${message}`;
  }
  const data = result.result || {};
  const tasks = Array.isArray(data.tasks) ? data.tasks : [];
  const total = Number(data.totalCount == null ? tasks.length : data.totalCount);
  const lines = [`<b>${tg.escapeHtml(title)}</b> · ${tg.escapeHtml(organization)}`, `Всего: <b>${Number.isFinite(total) ? total : tasks.length}</b>`];
  if (!tasks.length) {
...
function renderCash(result) {
  if (!result || result.ok !== true) {
    if (result && result.code === "FORBIDDEN") return "Нет доступа к финансам этой организации.";
...
function renderRisk(result) {
  if (!result || result.ok !== true) {
    if (result && result.code === "FORBIDDEN") return "Нет доступа к Horizon этой организации.";
  ...
  return [
    ...
    "<i>Это серверный ledger-runway, без выдуманной вероятности Monte-Carlo.</i>",
  ].join("\n");
}

function brief(e, ctx, app, organizationId) {
  const finance = financeBalance(e, ctx, organizationId);
  const horizon = horizonSnapshot(e, ctx, organizationId);
  const today = dueTasks(e, ctx, organizationId, "today", 3);
  const overdue = dueTasks(e, ctx, organizationId, "overdue", 3);
  const name = orgName(app, ctx, organizationId);

  const lines = [`<b>${tg.escapeHtml(name)}</b> · сейчас`];
  if (finance.ok === true) {
    lines.push(`Касса: <b>${tg.escapeHtml(tg.formatMoney(finance.result.currentBalance, finance.result.reportingCurrency))}</b>`);
  } else {
  ...
  if (horizon.ok === true) {
    ...
    const days = horizon.result.cushionDays == null ? "—" : `${horizon.result.cushionDays} дн.`;
    lines.push(`Риск: <b>${tg.escapeHtml(risk.label)}</b> · запас ${tg.escapeHtml(days)}`);
  } else {
    lines.push(horizon.code === "FORBIDDEN" ? "Риск: нет доступа к Horizon" : "Риск: данные недоступны");
  }
  if (today.ok === true) lines.push(`Задач сегодня: <b>${Number(today.result.totalCount || 0)}</b>`);
  else lines.push(today.code === "FORBIDDEN" ? "Задачи: нет доступа" : "Задачи: данные недоступны");
  if (overdue.ok === true) lines.push(`Просрочено: <b>${Number(overdue.result.totalCount || 0)}</b>`);
  return lines.join("\n");
}
...
function liveLink(app, telegramUserId) {
  ...
  const selected = store.selectVisibleOrganization(app, identity, stored.payload.activeOrganizationId);
  ...
  return {ok: true, stored: stored, identity: identity, organizationId: selected.id, organizations: selected.organizations};
}

function acceptUserRate(app, live) {
  const rate = tg.acceptRate(live.stored.payload.rateLimit, Date.now(), RATE_LIMIT, RATE_WINDOW_MS);
...
function handleCommand(e, cfg, live, chatId, command) {
  ...
  if (["brief", "cash", "risk", "today", "overdue"].indexOf(name) >= 0) sendTyping(cfg, chatId);

  if (name === "brief") {
    return sendMessage(cfg, chatId, brief(e, live.identity, e.app, organizationId), mainKeyboard(cfg, organizationId));
  }
  if (name === "cash") {
    return sendMessage(cfg, chatId, renderCash(financeBalance(e, live.identity, organizationId)), [
      [{text: "Сменить org", callback_data: tg.callback("orgs", "")}, {text: "Риск", callback_data: tg.callback("risk", "")}],
      [{text: "Открыть кассу", url: deepLinkUrl(cfg, "treasury", organizationId)}],
    ]);
  }
  if (name === "risk") {
    return sendMessage(cfg, chatId, renderRisk(horizonSnapshot(e, live.identity, organizationId)), [
      [{text: "Касса", callback_data: tg.callback("cash", "")}, {text: "Сменить org", callback_data: tg.callback("orgs", "")}],
      [{text: "Открыть Horizon", url: deepLinkUrl(cfg, "forecast", organizationId)}],
    ]);
  }
  if (name === "today" || name === "overdue") {
    const result = dueTasks(e, live.identity, organizationId, name, 8);
    const title = name === "today" ? "Задачи на сегодня" : "Просроченные задачи";
    return sendMessage(cfg, chatId, renderTaskList(title, result, orgName(e.app, live.identity, organizationId), 8), taskKeyboard(cfg, organizationId));
  }
  if (name === "org" || name === "orgs") {
    ...
    if (query) {
      ...
      if (selected.id && (selected.id === query || selected.organizations.some((item) => item.id === selected.id && item.name.toLowerCase() === query.toLowerCase()))) {
        live.stored.payload.activeOrganizationId = selected.id;
        store.updateLink(e.app, live.stored.payload);
        return sendMessage(cfg, chatId, `Текущая организация: <b>${tg.escapeHtml(orgName(e.app, live.identity, selected.id))}</b>`, mainKeyboard(cfg, selected.id));
      }
    }
    return sendMessage(cfg, chatId, "Выберите организацию. Бот покажет только те, к которым у вас есть доступ.", orgKeyboard(live.organizations, organizationId));
  }
  if (name === "help" || name === "start") {
    return sendMessage(cfg, chatId, helpText(true), mainKeyboard(cfg, organizationId));
  }
  return sendMessage(cfg, chatId, "Не понял команду. Используйте /brief, /cash, /risk, /today, /overdue или /org.", mainKeyboard(cfg, organizationId));
}
...
function handleStartLink(e, cfg, message, command) {
  ...
  if (!consumed.ok) {
    ...
    return sendMessage(cfg, chatId, expired ? "Код привязки истёк. Создайте новый в WesiOS." : "Код привязки недействителен. Создайте новый в WesiOS.");
  }
  const identity = store.resolveIdentityForAuth(e.app, consumed.payload.authUserId);
  if (!identity) return sendMessage(cfg, chatId, "Аккаунт WesiOS больше недоступен. Войдите в приложение и создайте новую привязку.");
  ...
  if (alreadyTg && String(alreadyTg.payload.authUserId || "") !== String(identity.authUserId || "")) {
    return sendMessage(cfg, chatId, "Этот Telegram уже привязан к другому аккаунту WesiOS. Сначала отвяжите его там.");
  }
  ...
  if (alreadyAuth && String(alreadyAuth.payload.telegramUserId || "") !== telegramUserId) {
    return sendMessage(cfg, chatId, "Аккаунт WesiOS уже привязан к другому Telegram. Отзовите старую привязку в приложении.");
  }

  const selected = visibleSelection(e.app, identity, consumed.payload.activeOrganizationId);
  if (!selected.ok) return sendMessage(cfg, chatId, "Привязка не завершена: у аккаунта нет доступа к выбранной организации.");
...
function handlePrivateMessage(e, cfg, message) {
  ...
  if (!acceptUserRate(e.app, live)) return sendMessage(cfg, chatId, "Слишком много команд подряд. Подождите минуту.");
...
function handleGroupMessage(e, cfg, message) {
  ...
  if (!chatId) return;
  // Groups never receive balances or task bodies in MVP. The bot remains a
  // private thin client until a dedicated group policy exists.
  return sendMessage(cfg, chatId, `Чувствительные данные WesiOS доступны только в личке с @${tg.escapeHtml(cfg.botUsername)}.`);
}
...
function handleCallback(e, cfg, query) {
  ...
  if (!chatId || !telegramUserId || !parsed) return answerCallback(cfg, query && query.id, "Кнопка устарела");
  const live = liveLink(e.app, telegramUserId);
  if (!live.ok) {
    answerCallback(cfg, query.id, "Привязка WesiOS недействительна");
    return sendMessage(cfg, chatId, helpText(false));
  }
  if (!acceptUserRate(e.app, live)) return answerCallback(cfg, query.id, "Слишком много запросов");

  if (parsed.action === "org") {
    const selected = visibleSelection(e.app, live.identity, parsed.value);
    if (!selected.ok) return answerCallback(cfg, query.id, "Нет доступа к этой организации");
    live.stored.payload.activeOrganizationId = selected.id;
    store.updateLink(e.app, live.stored.payload);
  ...
  return handleCommand(e, cfg, live, chatId, {name: parsed.action, args: parsed.value});
}

function notifierFingerprint(kind, organizationId, value) {
  return `${kind}:${organizationId}:${String(value || "")}`;
}
...
function runNotifier(app) {
  ...
  for (const row of links) {
    ...
    const offset = Number(prefs.timezoneOffsetMinutes == null ? payload.timezoneOffsetMinutes || 0 : prefs.timezoneOffsetMinutes);
    if (tg.isQuietHours(Date.now(), offset, prefs.quietFromHour, prefs.quietToHour)) {
      store.updateLink(app, payload);
      continue;
    }
    const event = {app: app};
    const fingerprints = payload.alertFingerprints && typeof payload.alertFingerprints === "object" ? payload.alertFingerprints : {};
    ...
    if (prefs.risk !== false) {
      ...
      if (riskResult.ok === true) {
        ...
        const fp = notifierFingerprint("risk", selected.id, risk.level + ":" + String(riskResult.result.cushionDays));
        if ((risk.level === "critical" || risk.level === "warning") && fingerprints.risk !== fp) {
          sendMessage(cfg, payload.telegramChatId, [
        ...
        fingerprints.risk = fp;
      }
    }

    if (prefs.overdue !== false) {
      const overdue = dueTasks(event, identity, selected.id, "overdue", 5);
      if (overdue.ok === true) {
        const count = Number(overdue.result.totalCount || 0);
        const fp = notifierFingerprint("overdue", selected.id, count);
        if (count > 0 && fingerprints.overdue !== fp) {
          sendMessage(cfg, payload.telegramChatId, renderTaskList("Просроченные задачи", overdue, orgName(app, identity, selected.id), 5), taskKeyboard(cfg, selected.id));
        }
        fingerprints.overdue = fp;
      }
    }

    payload.alertFingerprints = fingerprints;
...
routerAdd("POST", "/api/wesi/telegram/link/create", (e) => {
  ...
  if (!selected.ok) return e.json(403, {ok: false, code: selected.code, message: selected.message});
  const ticket = store.createLinkCode(e.app, identity, selected.id, body.timezoneOffsetMinutes);
  return e.json(200, {
    ok: true,
    linked: false,
    code: ticket.code,
...
routerAdd("POST", "/api/wesi/telegram/context", (e) => {
  ...
  return e.json(200, {ok: true, activeOrganizationId: selected.id, activeOrganizationName: orgName(e.app, identity, selected.id)});
}, $apis.requireAuth("users"));
...
routerAdd("POST", "/api/wesi/telegram/preferences", (e) => {
  ...
  if (!current) return e.json(404, {ok: false, code: "TELEGRAM_NOT_LINKED"});
  const body = e.requestInfo().body || {};
  const previous = current.payload.notificationPrefs && typeof current.payload.notificationPrefs === "object" ? current.payload.notificationPrefs : {};
  const next = Object.assign({}, previous);
  if (body.risk != null) next.risk = body.risk === true;
  if (body.overdue != null) next.overdue = body.overdue === true;
  if (body.quietFromHour != null) next.quietFromHour = Math.max(0, Math.min(23, Number(body.quietFromHour)));
  if (body.quietToHour != null) next.quietToHour = Math.max(0, Math.min(23, Number(body.quietToHour)));
  if (body.timezoneOffsetMinutes != null) next.timezoneOffsetMinutes = Math.max(-840, Math.min(840, Number(body.timezoneOffsetMinutes)));
...
routerAdd("GET", "/api/wesi/telegram/open", (e) => {
  const target = String(e.request.url.query().get("target") || "home").toLowerCase();
  const org = String(e.request.url.query().get("org") || "").replace(/[^A-Za-z0-9_.:-]/g, "").slice(0, 80);
  const allowed = {home: "home", treasury: "treasury", forecast: "treasury/forecast", tasks: "tasks", ai: "ai"};
...
routerAdd("POST", "/api/wesi/telegram/webhook", (e) => {
  ...
  if (!secret || secret !== cfg.webhookSecret) return e.json(401, {ok: false, code: "UNAUTHORIZED"});
  ...
  try {
    ...
    // Telegram retries on non-2xx. A valid update must be acknowledged even if
    // a downstream tool was temporarily unavailable; command handlers already
    // return explicit user-facing data errors where possible.
  }
  return e.json(200, {ok: true});
});

cronAdd(ALERT_CRON_ID, "*/5 * * * *", () => {
  try { runNotifier($app); }
  catch (_) {}
});
