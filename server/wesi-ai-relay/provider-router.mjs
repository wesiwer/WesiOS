...
function captureQuota(route, response, provider) {
  ...
  quotaSnapshots.set(route, {
    remainingPercent: known[0].remainingPercent,
    resetAt: known[0].resetAt,
...
function normalizedHistory(input) {
  ...
  for (const item of Array.isArray(input?.history) ? input.history : []) {
    ...
    if (!text) continue;
    result.push({role: String(item?.author || '') === 'user' ? 'user' : 'assistant', content: text});
  }
  result.push({role: 'user', content: String(input?.message || '')});
  return result;
}
...
async function callOpenAi(parsed, input, apiKey) {
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      model: parsed.model,
      messages: [{role: 'system', content: String(input?.system || '')}, ...normalizedHistory(input)],
    }),
...
async function callAnthropic(parsed, input, apiKey) {
  ...
  const response = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      messages: normalizedHistory(input),
    }),
    signal: AbortSignal.timeout(120000),
  });
...
export function quotaForRoutes(routes, keys = {}) {
  ...
  for (const [key, route] of Object.entries(routes || {})) {
    result[key] = describeRoute(route, keys);
  }
  return result;
}
...
export async function callProviderText(rawRoute, input, keys = {}, callGoogleText) {
  ...
  if (parsed.provider === 'google') {
    const result = await callGoogleText(parsed.model, input, keys.google);
    return result;
  }
