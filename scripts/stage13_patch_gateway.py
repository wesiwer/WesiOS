...
s = p.read_text()
old_import = "import {runPersonaCoagent} from './persona_coagent_orchestrator.mjs';\n"
new_import = old_import + "import {runDynamicSubagents} from './dynamic_subagent_orchestrator.mjs';\n"
if 'runDynamicSubagents' not in s:
    if old_import not in s:
        raise SystemExit('import anchor missing')
    s = s.replace(old_import, new_import, 1)

anchor = "      const toolResults = [];\n      const seenCalls = new Set();\n"
if 'WESI_AI_VERIFIED_DYNAMIC_SUBAGENT_RESULTS' not in s:
    if anchor not in s:
        raise SystemExit('lead loop anchor missing')
    block = '''      if (leadPrepared.subagents?.enabled === true) {
        try {
