...
- **Этапы 1–8/16: DONE.**
- **Текущий этап: 9/16 — Self-debug loop + validated artifact creation/build delivery.**
- Stage 9 реализуется в ветке `agent/wesi-ai-stage9-self-debug-artifacts`; focused Stage 6–9 analyze/test gate уже прошёл, но этап останется `IN PROGRESS` до полного PR gate, Android debug, Windows release и merge в `main`.
...
| 8/16 | **DONE** | Adaptive Resource Scheduler + durable jobs: L0–L4 classification, trusted workload registry, verified capability/pack/resource filters, CPU/RAM/VRAM/disk/thermal/power/concurrency policy, heavy foreground requirement, Control Plane heavy-compute prohibition, bounded durable journal, checkpoint/pause/resume, `waitingForWorker`, restart-safe coordinator integration. PR #170, validated head `f9bd4e28dc8b88363a4025c8ea0d4231f48432bb`, main `4fca3101927d4686e365aaf33530786ea7345b82`. Full PR check, Android debug и Windows release прошли. No production deploy/release. |
| 9/16 | **IN PROGRESS** | Bounded self-debug loop поверх Stage 6 Local Runtime + Stage 8 durable jobs: objective verify → diagnose → bounded repair → re-test, tool/iteration/wall-time budgets, fail-closed artifact validation, SHA-256 re-check before delivery и запрет объявлять успех до подтверждённой проверки/выдачи артефакта. Focused Stage 6–9 analyze/test gate passed; final PR/platform gates pending. |
...
1. Не переделывать Stage 1–8, если не найден воспроизводимый regression.
2. Stage 9 считается завершённым только после отдельного PR, полного repository gate, Android debug, Windows release и merge в `main`.
3. Каждый новый этап: отдельная ветка → focused tests → полный repository gate → Android debug → Windows release → merge в `main` → только после этого `DONE`.
4. Не запускать production release/deploy автоматически. Это отдельное явное действие.
5. Не переносить тяжёлые L3/L4 задачи на основной VPS/Control Plane. Если worker недоступен: checkpoint → `waitingForWorker` → resume после возвращения worker.
6. Старый PR #149 использовать только как исторический reference. Не мержить его как готовый stack: Stage 6–8 уже реализованы заново поверх актуального `main`.
