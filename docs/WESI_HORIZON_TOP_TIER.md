...
## Что исправлено при слиянии в main

Движок проверялся замером на данных, где доход ровно равен расходу: три
месяца по 1000 в день расхода и три зарплаты по 30 000. Правильный ответ —
ровная линия. Движок её не давал, и причин оказалось пять.

**Сегодняшний день шёл в статистику как полный.** Он ещё не кончился, и как
день истории занижал и уровень, и скорость: на ровном ряде движок видел
расход 989 вместо 1000 и наклон −13 в день, который к тридцатому дню срезал
ожидание до 808. Сегодняшние операции и так учтены в стартовом балансе.

**Свежесть мерялась днями, а не циклами потока.** Дневное затухание у
зарплаты раз в месяц даёт ровно двукратный разброс: 1370 в день сразу после
выплаты против 718 перед следующей. Теперь веса раздаются целым циклам.

**Наклон считался по сырым дням.** Регрессия по 21 дню у редкого крупного
дохода описывала не тренд, а положение единственной выплаты в окне: ±390 в
день, что двигало ожидание на половину уровня.

**Обрезка `max(0, …)` съедала минусы, оставляя плюсы.** Остаток берётся
бутстрапом из другого дня, чем текущее ожидание, и по неравенству Йенсена
среднее после обрезки строго выше истинного. Прогноз рос на 450…2500 в день
там, где должен был стоять. Недобранное теперь переносится на следующий
день: каждый день неотрицателен, сумма за траекторию сохраняется.

**Регулярный расход считался дважды** — как проведённые копии в истории и
как проекция вперёд. Аренда 30 000 в месяц меняла прогноз на 55 500.

Итог замера: разброс по фазе месяца упал с 89 361 до 2 606, при дефиците
500 в день прогноз показывает минус, а не плюс.

## Что добавлено сверх исходной ветки

- **Платёжный календарь.** Месячные и годовые платежи ложатся на рабочие
  дни: списания уходят вперёд, зарплата по ТК — назад, накануне выходного.
  Правила статьи 112, а не таблица на конкретный год. Ежедневный и недельный
  ритм не двигается — это привычка тратить, а не расписание списаний.
- **Живые переходы режимов.** Смена режима перестала идти по общей недельной
  сетке, из-за которой все пять тысяч сценариев переключались одновременно.
- **Конформная калибровка по каждому хвосту отдельно.** Вместо одного
  множителя 0,80/покрытие — эмпирические квантили нормализованных промахов
  из ретро-проверки, порознь вниз и вверх, с поправкой (n+1)/n на конечную
  выборку.
- **Подушка в днях.** Рядом с суммой резерва — срок, на который его хватит,
  и отдельная метрика «денег хватит на», обе от реального расхода за день.
- **Выполнимые советы.** «Снизь расходы» сверяется с тем, сколько человек
  тратит: до четверти трат — обычный совет, до половины — «жёстко», выше —
  движок честно говорит, что одним урезанием не обойтись.

## Release state

Влито в `main` версией 0.20.0+69 вместе с исправлениями выше. Ветка
`agent/horizon-top-tier` больше не является отдельной линией разработки.
