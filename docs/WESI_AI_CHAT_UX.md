...
Прямой streaming transport добавляет промежуточные `meta/tool/agent/activity` события по мере выполнения. Если streaming gateway недоступен, клиент безопасно откатывается на основной `/api/wesi/ai/chat`: в «Думающем» во время ожидания остаётся фактический локальный этап подготовки, а после verified ответа в work log добавляются реальные tool results. Такой fallback не изображает скрытые мысли модели и не выдаётся за token-by-token reasoning.
