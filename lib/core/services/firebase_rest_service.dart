...
class FirebaseFailure {
  ...
  String describe({bool russian = true}) {
    ...
    return switch (code) {
      ...
      // Ошибки НАСТРОЙКИ проекта отделены от неверного пароля намеренно.
      // Одинаковый текст для «пароль не тот» и «вход по паролю вообще не
      // включён» превращает решаемую задачу в тупик: человек проверяет
      // пароль десятый раз, а дело совсем не в нём.
      'OPERATION_NOT_ALLOWED' =>
        'В проекте Firebase не включён вход по почте и паролю: '
            'Authentication → Sign-in method → Email/Password',
      'API_KEY_INVALID' || 'INVALID_API_KEY' =>
        'Ключ проекта Firebase недействителен — проверьте apiKey',
      'ADMIN_ONLY_OPERATION' =>
        'Регистрация закрыта: учётную запись создаёт владелец в консоли Firebase',
      'PERMISSION_DENIED' =>
