...
class _AnalyticsScreenState extends State<AnalyticsScreen> {
  ...
  Widget _emptyState() => _card(
        child: Column(
          children: [
            ...
            Text(
              _ru
                  ? 'Добавьте операции в «Финансы» или задачи — аналитика соберётся сама.'
                  : 'Add operations in Finance or some tasks — analytics builds itself.',
  ...
  Widget _kpiGrid(AnalyticsSnapshot d) {
    final tiles = <Widget>[
      _kpiTile(_ru ? 'Доходы' : 'Income', d.income, AppTheme.accentGreen, money: true),
      _kpiTile(_ru ? 'Расходы' : 'Expenses', d.expense, AppTheme.accentRed, money: true, higherIsBetter: false),
      _kpiTile(_ru ? 'Чистый' : 'Net', d.net, AppTheme.accent, money: true),
      _kpiTile(_ru ? 'Операций' : 'Operations', d.operations, const Color(0xFF38BDF8), money: false),
    ];
  ...
  Widget _kpiTile(String label, Kpi kpi, Color color, {required bool money, bool higherIsBetter = true}) {
    ...
    return _card(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            money ? CurrencyService.format(kpi.value) : kpi.value.toStringAsFixed(0),
          ...
          Row(
            children: [
              Icon(
                kpi.delta == 0 ? Icons.remove : (kpi.isUp ? Icons.arrow_upward : Icons.arrow_downward),
                size: 12,
                color: kpi.delta == 0 ? AppTheme.textMuted : (good ? AppTheme.accentGreen : AppTheme.accentRed),
              ),
  ...
  Widget _trendCard(AnalyticsSnapshot d) {
    ...
    final maxY = d.months.map((m) => m.income > m.expense ? m.income : m.expense).reduce((a, b) => a > b ? a : b);
    return _card(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
            ...
            child: BarChart(
              BarChartData(
                ...
                gridData: FlGridData(
                  ...
                  getDrawingHorizontalLine: (v) => FlLine(color: AppTheme.glassBorder.withOpacity(0.5), strokeWidth: 1),
                ),
                ...
                titlesData: FlTitlesData(
                  ...
                  leftTitles: AxisTitles(
                    sideTitles: SideTitles(
                      showTitles: true,
                      reservedSize: 46,
                      getTitlesWidget: (v, meta) => Text(_compact(v), style: TextStyle(fontSize: 9, color: AppTheme.textMuted)),
                    ),
                  ),
                  bottomTitles: AxisTitles(
                    sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (v, meta) {
                        final i = v.toInt();
                        if (i < 0 || i >= d.months.length) return const SizedBox.shrink();
                        final m = d.months[i].month;
                        return Padding(
                          padding: const EdgeInsets.only(top: 6),
                          child: Text('${m.month.toString().padLeft(2, '0')}.${m.year % 100}',
                ...
                barTouchData: BarTouchData(
                  touchTooltipData: BarTouchTooltipData(
                    ...
                    getTooltipItem: (group, gi, rod, ri) => BarTooltipItem(
                      ...
                      TextStyle(fontSize: 11, color: ri == 0 ? AppTheme.accentGreen : AppTheme.accentRed),
                    ),
                  ),
                ),
                barGroups: List.generate(d.months.length, (i) {
                  ...
                  return BarChartGroupData(
                    ...
                    barsSpace: 4,
                    barRods: [
                      BarChartRodData(toY: CurrencyService.display(m.income), color: AppTheme.accentGreen, width: 9, borderRadius: BorderRadius.circular(3)),
                      BarChartRodData(toY: CurrencyService.display(m.expense), color: AppTheme.accentRed, width: 9, borderRadius: BorderRadius.circular(3)),
                    ],
                  );
                }),
              ),
  ...
  Widget _categoriesCard(AnalyticsSnapshot d) {
    return _card(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _cardTitle(_ru ? 'Куда уходит и откуда приходит' : 'Where money moves'),
          const SizedBox(height: 14),
          if (d.topExpenseCategories.isEmpty && d.topIncomeCategories.isEmpty)
            Text(_ru ? 'Нет операций за период' : 'No operations in this period', style: TextStyle(fontSize: 12, color: AppTheme.textMuted)),
          if (d.topExpenseCategories.isNotEmpty) ...[
            Text(_ru ? 'Расходы' : 'Expenses', style: TextStyle(fontSize: 11, color: AppTheme.textMuted)),
            const SizedBox(height: 8),
            ..._bars(d.topExpenseCategories, AppTheme.accentRed),
            const SizedBox(height: 14),
          ],
          if (d.topIncomeCategories.isNotEmpty) ...[
            Text(_ru ? 'Доходы' : 'Income', style: TextStyle(fontSize: 11, color: AppTheme.textMuted)),
            const SizedBox(height: 8),
      ...
      return Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Expanded(child: Text(e.key, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 12, color: AppTheme.textSecondary))),
                Text(CurrencyService.format(e.value), style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppTheme.textPrimary)),
              ],
            ),
            const SizedBox(height: 5),
            ClipRRect(
              borderRadius: BorderRadius.circular(3),
              child: LinearProgressIndicator(value: share, minHeight: 5, backgroundColor: AppTheme.background, valueColor: AlwaysStoppedAnimation(color)),
            ),
          ],
        ),
      );
    ...
    return _card(
      child: Column(
        ...
        children: [
          ...
          if (d.tasksTotal == 0)
            Text(
              _ru ? 'Задач пока нет — создайте их во вкладке «Задачи».' : 'No tasks yet — create some in the Tasks tab.',
              style: TextStyle(fontSize: 12, color: AppTheme.textMuted),
            )
          else ...[
            Row(
              children: [
                ...
                Text('${(d.taskCompletion * 100).round()}%', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppTheme.accent)),
              ],
            ),
            const SizedBox(height: 14),
            _statRow(icon: Icons.check_circle_outline, color: AppTheme.accentGreen, label: _ru ? 'Закрыто за период' : 'Closed in period', value: '${d.tasksDone}'),
            _statRow(icon: Icons.warning_amber_rounded, color: d.tasksOverdue > 0 ? AppTheme.accentRed : AppTheme.textMuted, label: _ru ? 'Просрочено' : 'Overdue', value: '${d.tasksOverdue}'),
            _statRow(icon: Icons.list_alt, color: AppTheme.textMuted, label: _ru ? 'Всего задач' : 'Tasks in total', value: '${d.tasksTotal}'),
          ],
        ],
      ),
    );
  ...
  Widget _statRow({required IconData icon, required Color color, required String label, required String value}) =>
      Padding(
  ...
  Widget _cardTitle(String text) => Text(text, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: AppTheme.textPrimary));
