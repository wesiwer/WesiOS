...
class TopicChunker {
  ...
  static TopicBoundary _score(List<ChunkInput> messages, int i) {
    ...
    // Долгая пауза при общем словаре — это возвращение к разговору, а не
    // новый разговор.
    //
    // Ночь между «отгрузили паллеты» и «паллеты забрал» набирала на разрез
    // одной только паузой. Человек же видит одну тему, разорванную сном.
    // Пауза остаётся сильнейшим сигналом, но перестаёт быть достаточным,
    // когда после неё говорят теми же словами. Порог 0.7 — это примерно
    // треть общего словаря: заметное совпадение, случайно так не выходит.
    if (silence > 0.5 && lexical < 0.7) score *= 0.5;

    // Одна лишь смена словаря, без паузы и без слова-переключателя, должна
    // быть **полной**, чтобы о ней спрашивать.
    //
    // Иначе спокойный разговор об одном договоре набирал 0.36 — чуть выше
    // порога вопроса — просто потому, что соседние реплики описывают разные
    // стороны одного дела и общих слов в них мало. Вопрос «кажется, вы
    // говорили о другом» в середине связного разговора хуже, чем
    // неразделённый разговор: его задают часто, и на него перестают
    // отвечать.
    //
    // Настоящая смена темы без паузы этим не страдает: там общих слов нет
    // вовсе, и расхождение равно единице. Случай «одна тема, но словарь
    // сменился целиком» здесь не решается принципиально — по словам он
    // неотличим от смены темы, и это работа модели, а не арифметики.
    if (silence < 0.2 && marker == 0 && lexical < 0.95) score = 0;
  ...
  static double _lexicalGap(List<ChunkInput> messages, int i) {
    ...
    final gap = 1 - _cosine(_counts(before), _counts(after));

    // Уверенность в этом числе зависит от того, на скольких словах оно
    // посчитано.
    //
    // «Ок» → «Понял» не имеют общих слов, косинус равен нулю, и расхождение
    // получается **единицей** — максимальным из возможных. По весу этого
    // одного признака уже хватало на вопрос, и переписка вида
    // «Ок / Да / Понял / Хорошо» исправно рождала «кажется, вы говорили о
    // другом — разделить?». На пяти словах такой вывод не на чем строить.
    //
    // Поэтому расхождение ослабляется, пока слов мало, и работает в полную
    // силу, когда их достаточно с обеих сторон. Настоящая смена темы этим
    // не страдает: там по три содержательных сообщения с каждой стороны.
    final confidence = min(1.0, _distinct(before) / _minWordsForLexical) *
        min(1.0, _distinct(after) / _minWordsForLexical);
    return gap * confidence;
  }

  /// Сколько разных содержательных слов нужно с каждой стороны, чтобы
  /// сравнению словарей вообще можно было верить.
  static const double _minWordsForLexical = 4;

  static int _distinct(List<String> tokens) => tokens.toSet().length;
  ...
  /// Название для произвольного куска переписки.
  ///
  /// Нужно там, где границу поставил человек, а не алгоритм: блок есть, а
  /// названия у него взяться неоткуда. Считается тем же способом, что и у
  /// обычных кусков, — иначе разделённые вручную блоки выглядели бы чужими
  /// среди остальных.
  static TopicChunk chunkFor(List<ChunkInput> messages, int start, int end) =>
      _chunk(messages, start, end);
