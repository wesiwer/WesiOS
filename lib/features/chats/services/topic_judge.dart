...
///
/// **Личные чаты к модели не попадают никогда.** По договорённости их не
/// может прочитать даже владелец; отправка их наружу ради красивых
/// заголовков отменила бы всю договорённость разом. Для них остаётся
/// местная разметка — и это не запасной вариант, а единственно допустимый.
...
/// Кому можно показывать переписку ради разметки.
class TopicPrivacy {
  TopicPrivacy._();

  /// Можно ли отдать этот чат внешнему судье.
  ///
  /// Рабочий — да: владелец и так может его прочитать, и он же решает,
  /// какой моделью пользоваться. Личный — нет и никогда: там сквозное
  /// шифрование, и «только ради заголовков» его не отменяет.
  static bool mayLeaveDevice(ChatKind kind) => kind == ChatKind.work;

  /// Причина отказа — её видно в интерфейсе, чтобы отсутствие красивых
  /// названий в личных чатах не выглядело поломкой.
  static String explain(ChatKind kind, {bool russian = true}) {
    if (mayLeaveDevice(kind)) {
      return russian
          ? 'Темы размечает Wesi AI'
          : 'Topics are named by Wesi AI';
    }
    return russian
        ? 'Личный чат размечается только на устройстве — наружу он не '
            'уходит даже ради названий тем'
        : 'A personal chat is analysed on-device only';
  }
}
