...
class WesiAiBackupService {
  ...
  static void _preflightZip(Uint8List package) {
    ...
    if (diskNumber != 0 ||
      ...
      throw const FormatException(
          'Неподдерживаемая структура ZIP Wesi AI package');
    }
    ...
    for (var index = 0; index < entriesTotal; index++) {
      if (offset < 0 ||
        ...
        throw const FormatException(
            'Повреждён central directory Wesi AI package');
      }
      ...
      if (next > eocdOffset ||
        ...
        throw const FormatException(
            'Неподдерживаемая ZIP entry Wesi AI package');
      }
      String name;
      try {
        name = utf8
            .decode(Uint8List.sublistView(
                package, offset + 46, offset + 46 + nameLength))
            .replaceAll('\\', '/');
      } catch (_) {
        throw const FormatException(
            'Некорректное имя ZIP entry Wesi AI package');
      }
      ...
      if (declaredTotal > maxPackageBytes) {
        throw const FormatException(
            'ZIP Wesi AI package превышает лимит распаковки');
      }
