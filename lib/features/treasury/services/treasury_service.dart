...
class TreasuryService {
  ...
  Future<void> processRecurringPayments() async {
    ...
    for (final tx in recurring) {
      ...
      if (guard > 0) {
        ...
        // сохраняется явно — иначе он потеряется вместе со сдвигом даты и
        // месячная дата снова начнёт съезжать.
        await _putRecurringSystem(anchor.copyWith(
          date: lastDue,
  ...
  Future<ForecastResult> generateForecast({
    ...
    final all = OrganizationContext.scope == OrganizationScope.subtree
        ...
        : rawScoped;
    // Старт прогноза — баланс на конец сегодняшнего дня, а не сумма всего
    // подряд. Операции, датированные будущим, поднимали сегодняшний остаток:
    // аванс, назначенный на следующую неделю, увеличивал старт, а в свою
    // дату уже ничего не происходило — прогноз шёл завышенным ровно на их
    // сумму.
    final balance = balanceOnDay(
      DateTime.now(),
      rawScoped,
      await AccountService.reportingBalanceForOrganizations(
        forecastIds,
        rawScoped,
      ),
    );
