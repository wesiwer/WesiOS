...
class _AudioMiniPlayerState extends State<AudioMiniPlayer> {
    ...
    return ValueListenableBuilder<AudioPlayerState>(
      ...
      builder: (context, local, _) {
        return ValueListenableBuilder<SpotifyPlaybackState>(
          ...
          builder: (context, spotify, __) {
            final useLocal = local.beat != null &&
                (local.playing || !spotify.available || !spotify.connected);
            final useSpotify = !useLocal && spotify.connected && spotify.available;
            if (!useLocal && !useSpotify && local.beat == null) {
            ...
            return Positioned(
              ...
              child: GestureDetector(
                ...
                child: useSpotify
                    ? _spotifyPlayer(context, spotify)
                    : _localPlayer(context, local),
              ),
            );
          },
        );
  ...
  Widget _localPlayer(BuildContext context, AudioPlayerState state) {
  ...
  Widget _spotifyPlayer(BuildContext context, SpotifyPlaybackState state) {
    return _shell(
      context,
      Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Row(children: [
              ...
              child: ClipRRect(
                ...
                child: state.imageUrl != null
                    ? Image.network(
                        ...
                        height: 46,
                        fit: BoxFit.cover,
                        errorBuilder: (_, __, ___) => _spotifyFallback(),
                      )
                    : _spotifyFallback(),
              ),
            ),
            ...
            IconButton(
              onPressed: SpotifyConnectService.previous,
              icon: Icon(Icons.skip_previous_rounded, color: AppTheme.textPrimary),
            ),
            IconButton(
            ...
            IconButton(
              onPressed: SpotifyConnectService.next,
              icon: Icon(Icons.skip_next_rounded, color: AppTheme.textPrimary),
            ),
          ]),
          SliderTheme(
  ...
  Widget _spotifyFallback() => Container(
        width: 46,
        height: 46,
