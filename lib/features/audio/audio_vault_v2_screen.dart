...
class _AudioVaultV2ScreenState extends State<AudioVaultV2Screen> {
  ...
  Future<List<BeatEntry>> _beats() async {
    ...
    if (_authorId != null) list = list.where((b) => b.authorEmployeeId == _authorId).toList();
    ...
    switch (_sort) {
      ...
      case 'author':
        list.sort((a, b) =>
            (TeamService.byId(a.authorEmployeeId)?.displayName ?? '')
                .compareTo(TeamService.byId(b.authorEmployeeId)?.displayName ?? ''));
        break;
      case 'stage':
        ...
        Expanded(
          child: IndexedStack(
            ...
            children: [
              _archive(),
              _player(),
              AudioMusicHub(onLocalPlaybackStarted: () => setState(() => _tab = 1)),
            ],
          ),
        ),
      ]),
      ...
      child: Wrap(
        ...
        children: [
          SizedBox(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                prefixIcon: Icon(Icons.search, size: 19, color: AppTheme.textMuted),
                filled: true,
          ...
          DropdownButton<BeatStage?>(
            ...
            hint: Text('Все стадии', style: TextStyle(color: AppTheme.textMuted)),
            items: [
              const DropdownMenuItem<BeatStage?>(value: null, child: Text('Все стадии')),
              ...BeatStage.values.map((s) => DropdownMenuItem<BeatStage?>(value: s, child: Text(_stageName(s)))),
            ],
          ...
          DropdownButton<String?>(
            value: _authorId,
            dropdownColor: AppTheme.surface,
            hint: Text('Все авторы', style: TextStyle(color: AppTheme.textMuted)),
            items: [
              const DropdownMenuItem<String?>(value: null, child: Text('Все авторы')),
              ...employees.map((e) => DropdownMenuItem<String?>(value: e.id, child: Text(e.displayName))),
            ],
            onChanged: (v) => setState(() => _authorId = v),
          ),
          SizedBox(width: 72, child: TextField(controller: _minBpm, keyboardType: TextInputType.number, onChanged: (_) => setState(() {}), decoration: const InputDecoration(labelText: 'BPM от', isDense: true))),
          SizedBox(width: 72, child: TextField(controller: _maxBpm, keyboardType: TextInputType.number, onChanged: (_) => setState(() {}), decoration: const InputDecoration(labelText: 'до', isDense: true))),
          DropdownButton<String>(
            value: _sort,
            dropdownColor: AppTheme.surface,
            items: const [
              DropdownMenuItem(value: 'updated', child: Text('Недавно изменённые')),
              ...
              DropdownMenuItem(value: 'lease', child: Text('Ближайшее продление')),
            ],
          ...
          if (_stage != null || _authorId != null || _favoritesOnly || _minBpm.text.isNotEmpty || _maxBpm.text.isNotEmpty)
            TextButton.icon(onPressed: _resetFilters, icon: const Icon(Icons.filter_alt_off, size: 16), label: const Text('Сбросить')),
        ],
      ),
    );
  }
  ...
  Widget _emptyArchive() => Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text('Архив битов пуст', style: TextStyle(color: AppTheme.textPrimary, fontWeight: FontWeight.w800)),
          const SizedBox(height: 5),
          Text('Добавь первый бит и собери MP3 / WAV / Trackout / проект / документы в одной карточке.',
              textAlign: TextAlign.center,
              style: TextStyle(color: AppTheme.textMuted, fontSize: 12)),
          const SizedBox(height: 14),
          FilledButton.icon(onPressed: _createBeat, icon: const Icon(Icons.add), label: const Text('Добавить бит')),
        ]),
      );

  Widget _beatCard(BeatEntry beat, List<BeatEntry> queue) {
    final author = TeamService.byId(beat.authorEmployeeId)?.displayName ?? 'Не указан';
          ...
          Expanded(
            child: Stack(fit: StackFit.expand, children: [
              hasCover
                  ...
                  : Container(
                      decoration: BoxDecoration(gradient: LinearGradient(colors: [AppTheme.accent.withOpacity(.35), AppTheme.background])),
                      child: Icon(Icons.graphic_eq, size: 56, color: AppTheme.accent.withOpacity(.75)),
                    ),
              Positioned.fill(child: DecoratedBox(decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Colors.transparent, Colors.black.withOpacity(.76)])))),
              Positioned(
                right: 8,
                top: 8,
                child: IconButton.filledTonal(
                  onPressed: () => AudioVaultService.save(beat.copyWith(favorite: !beat.favorite)),
                  icon: Icon(beat.favorite ? Icons.star : Icons.star_border, color: beat.favorite ? Colors.amber : AppTheme.textPrimary),
                ),
              ),
              ...
              Positioned(
                ...
                bottom: 10,
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(beat.title, maxLines: 1, overflow: TextOverflow.ellipsis,
          ...
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Wrap(spacing: 6, runSpacing: 5, children: [
                ...
                if (beat.musicalKey.isNotEmpty) _pill(beat.musicalKey, AppTheme.textMuted),
                if (beat.lease != null) LeaseCountdown(lease: beat.lease!, compact: true),
              ]),
              ...
              Row(children: [
                ...
                if (beat.attachments.isNotEmpty) ...[
                  Icon(Icons.description_outlined, size: 15, color: AppTheme.textMuted),
                  const SizedBox(width: 3),
                  Text('${beat.attachments.length}', style: TextStyle(color: AppTheme.textMuted, fontSize: 9)),
                ],
              ]),
            ]),
          ),
  ...
  Widget _fileBadge(String label, String? path, BeatEntry beat) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: exists ? AppTheme.accentGreen.withOpacity(.11) : AppTheme.background.withOpacity(.32),
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: exists ? AppTheme.accentGreen.withOpacity(.3) : AppTheme.glassBorder),
        ),
        child: Text(label, style: TextStyle(fontSize: 9.5, fontWeight: FontWeight.w800, color: exists ? AppTheme.accentGreen : AppTheme.textMuted)),
      ),
    );
  }
  ...
  Widget _pill(String text, Color color) => Container(
        ...
        decoration: BoxDecoration(color: color.withOpacity(.12), borderRadius: BorderRadius.circular(999), border: Border.all(color: color.withOpacity(.25))),
  ...
  Widget _player() => ValueListenableBuilder<AudioPlayerState>(
        ...
        builder: (context, state, _) {
          ...
          return ListView(padding: const EdgeInsets.fromLTRB(16, 12, 16, 140), children: [
            Row(children: [
              Expanded(child: Text(beat?.title ?? 'Ничего не играет', style: TextStyle(color: AppTheme.textPrimary, fontSize: 18, fontWeight: FontWeight.w900))),
              DropdownButton<VisualizerMode>(
                value: _visualizer,
                dropdownColor: AppTheme.surface,
                items: VisualizerMode.values.map((v) => DropdownMenuItem(value: v, child: Text(_visualizerName(v)))).toList(),
                onChanged: (v) => setState(() => _visualizer = v ?? VisualizerMode.water),
              ),
            ]),
            const SizedBox(height: 12),
            ...
            Wrap(
              ...
              children: VisualizerMode.values
                  .map((v) => ChoiceChip(selected: _visualizer == v, onSelected: (_) => setState(() => _visualizer = v), label: Text(_visualizerName(v))))
                  .toList(),
            ),
          ]);
        },
  ...
  Future<void> _openBeat(BeatEntry beat) async {
    await Navigator.of(context).push(MaterialPageRoute(builder: (_) => _BeatDetailsV2(beatId: beat.id)));
    if (mounted) setState(() {});
  }

  String _stageName(BeatStage s) => switch (s) {
        BeatStage.idea => 'Идея', BeatStage.draft => 'Черновик', BeatStage.production => 'Продакшн', BeatStage.mixing => 'Сведение', BeatStage.mastering => 'Мастеринг', BeatStage.ready => 'Готов', BeatStage.negotiating => 'Переговоры', BeatStage.leased => 'Аренда', BeatStage.sold => 'Продан', BeatStage.exclusive => 'Эксклюзив', BeatStage.archived => 'Архив'
      };
  Color _stageColor(BeatStage s) => switch (s) {
        BeatStage.ready => AppTheme.accentGreen, BeatStage.negotiating => Colors.orange, BeatStage.leased => Colors.amber, BeatStage.sold || BeatStage.exclusive => Colors.purpleAccent, BeatStage.archived => AppTheme.textMuted, _ => AppTheme.accent
      };
  String _visualizerName(VisualizerMode v) => switch (v) {
        VisualizerMode.water => 'Water Tank', VisualizerMode.membrane => 'Мембрана', VisualizerMode.spectrum => 'Спектр', VisualizerMode.particles => 'Частицы', VisualizerMode.tunnel => 'Тоннель', VisualizerMode.aurora => 'Аврора', VisualizerMode.orbit => 'Орбиты', VisualizerMode.pulseGrid => 'Pulse Grid'
      };
}
...
class _BeatEditorV2 extends StatefulWidget {
  ...
  static Future<BeatEntry?> show(BuildContext context, {BeatEntry? initial}) =>
      showDialog<BeatEntry>(context: context, builder: (_) => _BeatEditorV2(initial: initial));
...
class _BeatEditorV2State extends State<_BeatEditorV2> {
  ...
  void initState() {
    ...
    bpm = TextEditingController(text: b == null || b.bpm == 0 ? '' : '${b.bpm}');
  ...
  @override
  void dispose() {
    title.dispose(); bpm.dispose(); key.dispose(); genre.dispose(); mood.dispose(); tags.dispose(); notes.dispose();
    super.dispose();
  }
        ...
        child: SingleChildScrollView(
          child: Column(children: [
            ...
            DropdownButtonFormField<BeatStage>(
              ...
              items: BeatStage.values.map((s) => DropdownMenuItem(value: s, child: Text(_stageRu(s)))).toList(),
            ...
            const SizedBox(height: 10),
            TextField(controller: notes, minLines: 3, maxLines: 7, decoration: const InputDecoration(labelText: 'Заметки', alignLabelWithHint: true)),
          ]),
        ),
      ),
      actions: [
        TextButton(onPressed: () => Navigator.pop(context), child: const Text('Отмена')),
        FilledButton(onPressed: title.text.trim().isEmpty ? null : _save, child: Text(widget.initial == null ? 'Создать' : 'Сохранить')),
      ],
    );
  }

  void _save() {
    final tagList = tags.text.split(',').map((e) => e.trim()).where((e) => e.isNotEmpty).toSet().toList();
  ...
  Widget _field(TextEditingController c, String label, {bool number = false}) => TextField(
        controller: c,
...
class _BeatDetailsV2State extends State<_BeatDetailsV2> {
  ...
  Widget build(BuildContext context) {
    final b = beat;
    if (b == null) {
      return Scaffold(backgroundColor: AppTheme.background, body: const Center(child: CircularProgressIndicator()));
    }
    final author = TeamService.byId(b.authorEmployeeId)?.displayName ?? '—';
    return Scaffold(
      backgroundColor: AppTheme.background,
      appBar: AppBar(
        ...
        title: Text(b.title),
        actions: [
          IconButton(onPressed: () => _edit(b), icon: const Icon(Icons.edit_outlined), tooltip: 'Редактировать'),
          IconButton(onPressed: () => _delete(b), icon: Icon(Icons.delete_outline, color: AppTheme.accentRed)),
        ],
      ),
      body: ListView(
        ...
        children: [
          LayoutBuilder(builder: (context, c) {
            ...
            return wide
                ? Row(crossAxisAlignment: CrossAxisAlignment.start, children: [SizedBox(width: 340, child: cover), const SizedBox(width: 16), Expanded(child: info)])
            ...
            child: b.coverPath != null && File(b.coverPath!).existsSync()
                ...
                : Container(
                    ...
                    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Icon(Icons.add_photo_alternate_outlined, size: 54, color: AppTheme.accent),
                      const SizedBox(height: 8),
                      Text('Добавить обложку', style: TextStyle(color: AppTheme.textMuted)),
                    ]),
                  ),
          ),
        ),
  ...
  Widget _info(BeatEntry b, String author) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(child: Text(b.title, style: TextStyle(color: AppTheme.textPrimary, fontSize: 24, fontWeight: FontWeight.w900))),
            IconButton(onPressed: () => _edit(b), icon: const Icon(Icons.edit_outlined)),
          ]),
          ...
          if (b.notes.isNotEmpty) ...[
            const SizedBox(height: 14),
            Text(b.notes, style: TextStyle(color: AppTheme.textSecondary, height: 1.45)),
          ],
        ]),
      );
  ...
  Widget _fileRow(BeatEntry b, String title, BeatFileKind kind, String? path) {
    ...
    return ListTile(
      ...
      leading: Icon(exists ? Icons.check_circle : Icons.upload_file, color: exists ? AppTheme.accentGreen : AppTheme.textMuted),
      title: Text(title),
      subtitle: Text(exists ? File(path).path.split(Platform.pathSeparator).last : 'Не загружен', maxLines: 1, overflow: TextOverflow.ellipsis),
      trailing: Row(mainAxisSize: MainAxisSize.min, children: [
        IconButton(tooltip: exists ? 'Заменить' : 'Загрузить', onPressed: () => _importCore(b, kind), icon: const Icon(Icons.upload_rounded)),
        if (exists)
          IconButton(tooltip: 'Достать / отправить', onPressed: () => AudioVaultService.sharePath(path, subject: '${b.title} — $title'), icon: Icon(Icons.ios_share, color: AppTheme.accent)),
      ]),
    );
  }
    ...
    return _section(
      ...
      l == null
          ...
          : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Expanded(child: Text(l.artistName, style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 16))),
                LeaseCountdown(lease: l),
              ]),
              if (l.socialUrl.isNotEmpty)
                TextButton.icon(onPressed: () => launchUrl(Uri.parse(l.socialUrl), mode: LaunchMode.externalApplication), icon: const Icon(Icons.link), label: Text(l.socialUrl)),
              Text('Срок: ${_date(l.startsAt)} → ${_date(l.endsAt)}'),
              if (l.amount > 0) Text('Сумма: ${l.amount.toStringAsFixed(0)} ${l.currency}'),
              if (l.notes.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 6), child: Text(l.notes)),
              const SizedBox(height: 8),
              Row(children: [
                OutlinedButton.icon(onPressed: () => _editLease(b), icon: const Icon(Icons.edit_outlined), label: const Text('Продлить / изменить')),
  ...
  Widget _documents(BeatEntry b) => _section(
        ...
        Column(children: [
          Align(alignment: Alignment.centerLeft, child: OutlinedButton.icon(onPressed: () => _addAttachment(b), icon: const Icon(Icons.attach_file), label: const Text('Прикрепить документ / договор'))),
          for (final a in b.attachments)
            ListTile(
              ...
              trailing: IconButton(onPressed: () => AudioVaultService.sharePath(a.path, subject: '${b.title} — ${a.name}'), icon: const Icon(Icons.ios_share)),
            ),
        ]),
      );
  ...
  Widget _comments(BeatEntry b) => _section(
        ...
        Column(children: [
          Row(children: [
            Expanded(child: TextField(controller: comment, decoration: const InputDecoration(hintText: 'Например: договор отправлен, ждём ответ артиста…'))),
            const SizedBox(width: 8),
            IconButton.filled(
              onPressed: () async {
          ...
          for (final c in b.comments.reversed)
            ListTile(
              contentPadding: EdgeInsets.zero,
              leading: const CircleAvatar(child: Icon(Icons.person_outline, size: 17)),
              title: Text(c.text),
              subtitle: Text('${TeamService.byId(c.authorId)?.displayName ?? 'WesiOS'} · ${_dateTime(c.createdAt)}'),
            ),
        ]),
      );
  ...
  Widget _section(String title, IconData icon, Widget child) => Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(color: AppTheme.surface.withOpacity(.45), borderRadius: BorderRadius.circular(18), border: Border.all(color: AppTheme.glassBorder)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [Icon(icon, color: AppTheme.accent), const SizedBox(width: 9), Text(title, style: TextStyle(color: AppTheme.textPrimary, fontWeight: FontWeight.w900, fontSize: 15))]),
          const SizedBox(height: 12),
          child,
        ]),
      );
  ...
  Future<void> _addAttachment(BeatEntry b) async {
    final file = await AudioVaultService.importAttachment(beatId: b.id, kind: BeatFileKind.document);
    if (file == null) return;
  ...
  Future<void> _delete(BeatEntry b) async {
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('«${b.title}» и его локальные копии файлов Audio Vault будут удалены. Ableton project по исходному пути не удаляется.'),
        actions: [
...
class _LeaseDialogV2 extends StatefulWidget {
  ...
  static Future<BeatLease?> show(BuildContext context, BeatLease? initial) =>
      showDialog<BeatLease>(context: context, builder: (_) => _LeaseDialogV2(initial));
...
class _LeaseDialogV2State extends State<_LeaseDialogV2> {
  ...
  void initState() {
    ...
    amount = TextEditingController(text: l == null || l.amount == 0 ? '' : l.amount.toStringAsFixed(0));
  ...
  @override
  void dispose() {
    artist.dispose(); social.dispose(); amount.dispose(); notes.dispose();
    super.dispose();
  }
  ...
  Widget build(BuildContext context) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: SingleChildScrollView(
            child: Column(children: [
              TextField(controller: artist, onChanged: (_) => setState(() {}), decoration: const InputDecoration(labelText: 'Исполнитель / покупатель')),
              const SizedBox(height: 10),
              TextField(controller: social, decoration: const InputDecoration(labelText: 'Ссылка на соцсеть')),
              const SizedBox(height: 10),
              TextField(controller: amount, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Сумма, RUB')),
              const SizedBox(height: 10),
              Row(children: [
                Expanded(child: ListTile(title: const Text('Начало'), subtitle: Text(_d(start)), onTap: () => _pick(true))),
                Expanded(child: ListTile(title: const Text('Окончание / продление'), subtitle: Text(_d(end)), onTap: () => _pick(false))),
              ]),
              const SizedBox(height: 10),
              TextField(controller: notes, minLines: 2, maxLines: 5, decoration: const InputDecoration(labelText: 'Комментарий к сделке')),
              const SizedBox(height: 8),
              Row(children: [
                Icon(Icons.calendar_month, color: AppTheme.accent),
                const SizedBox(width: 8),
                const Expanded(child: Text('Дата окончания автоматически синхронизируется с Tasks/Calendar. При продлении дедлайн обновится.', style: TextStyle(fontSize: 11))),
              ]),
            ]),
          ),
        ),
