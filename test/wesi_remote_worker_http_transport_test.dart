...
void main() {
  test(
      'user pairing transport applies WesiOS auth and never puts poll secret in ticket',
      () async {
  ...
  test('worker heartbeat is HMAC signed over payloadJson and not user auth',
      () async {
  ...
  test(
      'user worker list converts server heartbeats into trusted remote profiles',
      () async {
    final now = DateTime.utc(2026, 8, 15, 17);
