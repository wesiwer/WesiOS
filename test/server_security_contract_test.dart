...
void main() {
  ...
  test('в фильтрах нет склейки строк', () {
    ...
    // Шаблон намеренно широкий. Прошлая версия проверки искала строку,
    // начинающуюся сразу с `"поле='`, и молча пропускала шесть настоящих
    // склеек вида `"coll='system' && rid='" + linkRid`: перед закрывающей
    // кавычкой стояло целое условие, а не одно поле. Проверка, которая
    // проходит на дырявом коде, хуже отсутствующей — она даёт уверенность.
    //
    // Теперь ловится сама форма подстановки: кавычка строки закрывается
    // сразу после открытой одинарной, а дальше идёт конкатенация.
    final concat = RegExp(r'''='"\s*\+|\+\s*"'\s*&&''', multiLine: true);
