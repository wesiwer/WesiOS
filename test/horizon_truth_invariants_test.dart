  ...
  test('runtime wiring never auto-posts recurring income as actual cash', () {
    final treasury =
        File('lib/features/treasury/services/treasury_service.dart')
            .readAsStringSync();
    // Проводится только расход. Поступление ждёт подтверждения фактом:
    // реальный приход вносится отдельно и служит доказательством того, что
    // платёж действительно исполнен.
    expect(treasury, contains('if (tx.type == TransactionType.expense)'));
    // Внутри ветки, которая создаёт транзакцию, не должно быть ни одного
    // упоминания дохода — иначе поступление начало бы проводиться само.
    final branch = treasury.substring(
      treasury.indexOf('if (tx.type == TransactionType.expense)'),
    );
    final body = branch.substring(0, branch.indexOf('if (guard > 0)'));
    expect(body.contains('TransactionType.income'), isFalse,
        reason: 'поступление не проводится автоматически как полученные деньги');
