...
def ensure_insert(text: str, needle: str, addition: str, marker: str, label: str) -> str:
    if marker in text:
        return text
    if needle not in text:
        raise SystemExit(f'{label} insertion needle not found')
...
queue_text = ensure_replace(queue_text, restore_old, restore_new, 'Restore state invariants')
...
if "bool _checkpointMatchesCurrentState" not in queue_text:
    helper_needle = "void _validateRequirements(WesiJobRequirements value) {\n"
    if helper_needle not in queue_text:
        raise SystemExit('Checkpoint helper insertion needle not found')
    queue_text = queue_text.replace(helper_needle, helper + helper_needle, 1)
